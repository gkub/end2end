// relay.cpp
// This is a simple relay server that relays messages between two clients.
...
int main() {
  ...
  // Set socket options
  int opt = 1;
  if (setsockopt(serverSocketDescriptor, SOL_SOCKET, SO_REUSEADDR, &opt,
                 sizeof(opt)) < 0) {
  ...
  // Bind socket to port
  if (bind(serverSocketDescriptor, (struct sockaddr *)&relayServerAddress,
  ...
  std::cout << "Relay server listening on port " << RELAY_PORT << "..."
            << std::endl;
  ...
  for (int i = 0; i < MAX_CLIENTS; i++) {
    ...
    clients[i].socket = accept(serverSocketDescriptor,
  ...
  while (true) {
    for (int i = 0; i < MAX_CLIENTS; i++) {
      ...
      if (!recvAll(clients[i].socket, buffer.data(), header.dataSize)) {
        std::cout << "Client " << i + 1 << " disconnected during data transfer."
                  << std::endl;
        close(clients[i].socket);
        clients[i].connected = false;
        continue;
      ...
      if (clients[otherClient].connected) {
        ...
        if (!sendAll(clients[otherClient].socket, &header, sizeof(header))) {
          std::cout << "Failed to send header to client " << otherClient + 1
                    << std::endl;
        ...
        // Then send the data
        if (!sendAll(clients[otherClient].socket, buffer.data(),
                     header.dataSize)) {
          std::cout << "Failed to send data to client " << otherClient + 1
                    << std::endl;
          close(clients[otherClient].socket);
          clients[otherClient].connected = false;
          continue;
        ...
        std::cout << "Relayed message from client " << i + 1 << " to client "
                  << otherClient + 1 << std::endl;
      }
    }
